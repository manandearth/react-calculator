...
(defn digit-pressed [digit]
  (if (= 0 (:display @app-state))
    (swap! app-state update :display #(long (str % digit)))
    (swap! app-state update :display #(double (str % digit)))))

(defn dot-pressed [digit]
  (let [display (str (:display @app-state))]
    (if-not (str/includes? display  ".")
      (swap! app-state update :display #(double (str % digit)))))
    )
(type (:display @app-state))
...
(defn compute [result & [op num & xs]]
  (case op
    "+" (recur (+ (js/parseFloat result) (js/parseFloat num)) xs)
    "-" (recur (- result num) xs)
    "/" (recur (/ result num) xs)
    "*" (recur (* result num) xs)
    result))
             ...
             :render (fn [props]
                       (element "div" {}
                                ...
                                (element "div" {}
                                         ...
                                         (element Button {:label "." :onPress dot-pressed})
                                         (element Button {:label "=" :onPress equals-pressed})
                                         (element Button {:label "+" :onPress operator-pressed}))
                                (element "div" {}
                                         (element Button {:label "C" :onPress clear-pressed}))))))
